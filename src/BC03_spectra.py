...
for j in range(18):
    ...
    record=False
    prev=0.
    for i in dat: # This reading process might seem weird, but the spectra files are generated by Le Phare, so I have to cater to their format.
        if record:
            if float(i[0])-prev<0.:
                break
            spec.append(i)
...
for j in range(0,6):
    ...
    for i in spec_sto[j]:
        ...
        ys.append(i[1]+5*np.log((1.+z)/3)) # Account for the scale factor, delta_mag = 5*log(d)
